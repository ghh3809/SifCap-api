...
import com.kotoumi.sifcapapi.model.vo.service.EventRequest;
...
public class RankUpdater implements Runnable {
    ...
    private static final String PLAYER_URL = "http://prod.game1.ll.sdo.com/main.php/ranking/eventPlayer";
    private static final String LIVE_URL = "http://prod.game1.ll.sdo.com/main.php/ranking/eventLive";
    ...
    public void run() {
        ...
        try {
            // 当前正在进行的活动信息
            Integer eventId = llProxyMapper.getCurrentEventId();
            ...
            for (int rank : PLAYER_RANK) {
                List<EventRequest> eventRequestList = llProxyMapper.getEventRequests(eventId, type, rank);
                for (EventRequest eventRequest : eventRequestList) {
                    ...
                    try {
                        if (updateRank(requestHeaders, requestData, eventId, type, rank, eventRequest.getId())) {
                            break;
                        }
                    } catch (Exception e) {
     ...
     * @return 是否更新成功
     */
    private boolean updateRank(String requestHeaders, String requestData, int eventId, String type, int rank, long id) {
        ...
        try (CloseableHttpResponse response = httpclient.execute(httpPost)) {
            if (response.getStatusLine().getStatusCode() != 200) {
                log.error("Update error, delete invalid request: {}", id);
                llProxyMapper.deleteEventRequest(id);
                return false;
            }
        ...
        // 导入数据库
        llProxyMapper.insertEventRank(eventId, type, rank, score);
        return true;
